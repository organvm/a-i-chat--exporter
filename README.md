...
> **Status — first revenue slice implemented, sovereign rail.** The Pro gate fails closed against an offline MONETA-signed key, captures checkout-return license keys, scrubs license material from the URL, and gates bulk / multi-provider export through `PRO_FEATURES`. The tracked userscript distribution is built against `https://mint.4444j99.dev`; live Claude/Gemini extraction remains foundation-only — see [Architecture: providers](#architecture-providers).
